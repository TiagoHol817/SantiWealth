...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="card rounded-xl p-3 shadow-2xl" style={{ minWidth: '160px' }}>
      <p className="text-muted" style={{ fontSize: '11px', marginBottom: '6px' }}>{label}</p>
...
function BurnRateChart({ data, promedio }: { data: Mes[]; promedio: number }) {
  ...
  return (
    <div className="card card-red p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <p className="text-white font-semibold">Burn Rate mensual</p>
          <p className="text-muted" style={{ fontSize: '13px', marginTop: '2px' }}>Tasa de consumo de gastos operacionales</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div style={{ width: '10px', height: '10px', borderRadius: '2px', backgroundColor: '#ef4444' }} />
            <span className="text-muted" style={{ fontSize: '12px' }}>Gastos</span>
          </div>
          <div className="flex items-center gap-2">
            <div style={{ width: '10px', height: '2px', backgroundColor: '#f59e0b' }} />
            <span className="text-muted" style={{ fontSize: '12px' }}>Promedio</span>
          </div>
        </div>
      </div>
