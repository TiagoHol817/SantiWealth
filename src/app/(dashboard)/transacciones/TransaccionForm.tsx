...
const CATEGORIAS_DEUDA   = ['Apartamento Flandes','Tarjeta crédito','Préstamo','Otro']
...
export default function TransaccionForm({ accounts }: { accounts: any[] }) {
  ...
  const lbl = { color: '#6b7280', fontSize: '11px', marginBottom: '4px', display: 'block' as const,
      ...
      <div className="fixed z-50 rounded-2xl p-6 w-full max-w-md shadow-2xl"
        ...
        <div className="flex gap-2 mb-5">
          {[
            ...
            <button key={val} onClick={() => { set('type', val); set('category', val === 'income' ? 'Salario' : val === 'debt_payment' ? 'Apartamento Flandes' : 'Alimentación') }}
        ...
        <button onClick={guardar} disabled={saving || !form.amount}
          ...
          style={{ backgroundColor: '#00d4aa', color: '#000', opacity: !form.amount ? 0.5 : 1 }}>
