/**
 * /api/accounts/[id]/hard-delete
 *   DELETE → permanently removes the account. Only allowed if already in trash.
 */
...
export async function DELETE(req: NextRequest, ctx: RouteCtx) {
  ...
  if (!account.deleted_at) {
    return NextResponse.json({ error: 'La cuenta debe estar en la papelera' }, { status: 400 })
  }

  const { error } = await supabase.from('accounts').delete().eq('id', id).eq('user_id', user.id)
  if (error) {
    console.error('[accounts hard-delete]', error.code)
